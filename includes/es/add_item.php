<section id="add">
  ...
  <button type="button" id="button_show" title="Abrir formulario para agregar item" data-action="open">
  ...
  <form action="<?php echo constant('URL').$lenguage; ?>/items/newItem" method="POST" name="form_item" id="form_add" class="form1" enctype="multipart/form-data">
    <ol>
      ...
      <li>
        <select name="add_type" id="add_type" title="Tipo">
          <option value="">Tipo*</option>
          <?php $all = true; include 'includes/'.$lenguage.'/type_options.php'; ?>
        </select>
      </li>
      <li>
        <input type="text" name="add_name" id="add_name" placeholder="Objeto*" title="Nombre">
      </li>
      <li>
        <textarea name="add_description" cols="30" rows="3" id="add_description" placeholder="Descripcion" title="Descripcion"></textarea>
      </li>
      <input type="file" name="new_icon" id="new_icon" hidden>

      <li class="console">
        <button type="button" id="add_item" title="Agregar objeto">
          <svg class="svg-form"><path d="M19 11h-6V5h-2v6H5v2h6v6h2v-6h6z"></path></svg>
        </button>
        <button type="button" id="reset_form_add" title="Resetear formulario">
