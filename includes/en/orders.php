...
for ($t_orders=0; $t_orders < sizeof($orders_list); $t_orders++) {
    ...
    for ($c_orders=0; $c_orders < sizeof($columns); $c_orders++) {
        switch ($columns[$c_orders]) {
            ...
            case 'user':
                echo "<td>";
                echo '<a href="'.constant("URL").'store/id/'.$orders_list[$t_orders]['id_user'].'" title="Go to '.$orders_list[$t_orders]['nick']."'".'s store">'.$orders_list[$t_orders]['nick'].'</a>';
                echo "</td>";
                echo "</td>";
                break;
            case 'date':
    ...
    if ($user != "") {
        if (($delete || $user->getRole() == "admin") && isset($orders_list[$t_orders]['id_order'])) {
            ...
            title="Delete order '.$idOrder.'" 
            class="emergent_form" 
            data-id="'.$idOrder.'"
            data-action="'.constant('URL').$lenguage.$action.'"
            data-msg="Delete order '.$idOrder.'?"
            data-name="id_order">';
        ...
        if ($user->getRole() == "admin") {
            ...
            echo '<button type="button" 
            title="Delete item '.$idItem.'" 
            class="emergent_form" 
            data-id="'.$idItem.'"
            data-action="'.constant('URL').$lenguage.$action.'"
            data-msg="Delete item '.$idItem.'?"
            data-name="id_item">';
            echo '<svg class="delete">
